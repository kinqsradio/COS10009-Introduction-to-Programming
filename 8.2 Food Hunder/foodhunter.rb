...
class Food
  ...
  attr_accessor :x, :y, :type, :image, :vel_x, :vel_y, :angle, :x, :y, :score, :direction, :smoke
...
def direction food
  speed = Math.sqrt(food.vel_x**2 + food.vel_y**2)
  if rand < 0.0025
    food.angle = rand(-180..180)
    @smoke_time = Time.now + 0.5
  end
end
...
def draw_food food
  if @smoke_time == nil or @smoke_time < Time.now
    food.image.draw_rot(food.x, food.y, ZOrder::FOOD, food.angle)
    @smoke_time == nil
  else
    food.smoke.draw_rot(food.x, food.y, ZOrder::FOOD, food.angle)
  end
end
...
class FoodHunterGame < (Example rescue Gosu::Window)
  ...
  def draw
    ...
    draw_hunter @player
    @all_food.each { |food| direction food; draw_food food }
