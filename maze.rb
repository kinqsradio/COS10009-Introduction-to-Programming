...
class GameWindow < Gosu::Window
  ...
  def cell_show(array)
    ...
    while (column_index < x_cell_count)
      row_index = 0
      while (row_index < y_cell_count)
        puts "Cell x: #{column_index}, y: #{row_index} north: #{run_cell_check(array[column_index][row_index].north).to_s} south: #{run_cell_check(array[column_index][row_index].south).to_s} east: #{run_cell_check(array[column_index][row_index].east).to_s} west: #{run_cell_check(array[column_index][row_index].west).to_s}"
        row_index += 1
      end
      puts "-------- End of Column ---------"
      column_index += 1
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      @north = MAP_WIDTH / CELL_DIM
      @south = MAP_HEIGHT / CELL_DIM - 1
      @east = MAP_HEIGHT / CELL_DIM
      @west = MAP_HEIGHT / CELL_DIM - 1

      if @north == [@vacant = true]
        north_path = @columns[column_index][row_index - 1]
        @visited = true
      end
